...
class EvolutionClient:
    ...
    def _http(self) -> httpx.AsyncClient:
        if self._client is None:
            raise RuntimeError("Use connect()/disconnect() antes de chamar a Evolution API.")
        return self._client
    ...
    async def create_instance(
        ...
        return self._parse_instance_info(instance_name, resp.json())
    ...
    def _parse_instance_info(instance_name: str, data: dict[str, Any]) -> InstanceInfo:
        state_raw = data.get("instance", {}).get("state") or data.get("hash", {}).get("state") or "close"
        try:
        ...
        if qr_data and isinstance(qr_data, dict):
            qrcode = QRCodeData(code=qr_data.get("code", ""), base64=qr_data.get("base64", ""))
